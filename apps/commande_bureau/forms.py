# apps/commande_bureau/forms.py
from django import forms
from django.forms import inlineformset_factory
from .models import CommandeBureau, LigneCommandeBureau, DesignationBureau, DescriptionBureau
from apps.fournisseurs.models import Fournisseur

class CommandeBureauForm(forms.ModelForm):
    class Meta:
        model = CommandeBureau
        fields = [
        ...
        widgets = {
            'mode_passation': forms.Select(attrs={'class': 'w-full border rounded px-3 py-2'}),
            'numero_commande': forms.TextInput(attrs={'class': 'w-full border rounded px-3 py-2'}),
            'fournisseur': forms.Select(attrs={'class': 'w-full border rounded px-3 py-2'}),
...
class LigneCommandeBureauForm(forms.ModelForm):
    class Meta:
        model = LigneCommandeBureau
        fields = ['designation', 'description', 'quantite', 'prix_unitaire']
        widgets = {
            'designation': forms.Select(attrs={'class': 'w-full border rounded px-3 py-2'}),
            'description': forms.Select(attrs={'class': 'w-full border rounded px-3 py-2'}),
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['designation'].queryset = DesignationBureau.objects.all().order_by('nom')
        # Les descriptions seront chargées dynamiquement via AJAX
        self.fields['description'].queryset = DescriptionBureau.objects.none()

# Formset pour gérer plusieurs lignes de commande
LigneCommandeBureauFormSet = inlineformset_factory(
    CommandeBureau,
    LigneCommandeBureau,
    form=LigneCommandeBureauForm,
